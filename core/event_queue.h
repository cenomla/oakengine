...
namespace oak {
	...
	public:
		...
		public:
			...
			const_iterator& operator++() {
				...
				if (ptrutil::add(ptr_, EVENT_SIZE) > ptrutil::add(header_, header_->size)) {
					header_ = static_cast<const detail::Block*>(header_->nextBlock);
					ptr_ = static_cast<const detail::Block*>(ptrutil::add(header_, sizeof(detail::Block)));
				}
		...
		EventQueue() : 
		allocator_{ &oalloc_freelist, 1024 * EVENT_SIZE, 8 },
		size_{ 0 } {}
