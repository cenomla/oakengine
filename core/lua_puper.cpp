...
namespace oak {
	...
	void lua_pup(PuperIo io, lua_State *L, int index, T& data, const ObjInfo& info) {
		if (io == PuperIo::IN) {
			...
			if (!luah::isNil(L, -1)) {
				if ((info.flags & ObjInfo::LENGTH) == ObjInfo::LENGTH) {
					lua_len(L, -1);
					p = 2;
				}
				data = luah::toValue<T>(L, -1);
			} else if ((info.flags & ObjInfo::LENGTH) == ObjInfo::LENGTH) {
				lua_len(L, index);
				p = 2;
				data = luah::toValue<T>(L, -1);
			}
	...
	void LuaPuper::pup(void*& data, const ObjInfo& info) {
		lua_pup<void*>(io_, L_, index_, data, info);
	}
