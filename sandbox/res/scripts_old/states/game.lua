...
local game = {
	...
	on_start = function(self, load)
		...
		table.insert(self.window.children, make_button(0.0, 0, 0, "spr_button", 96, 32, "Load",
			function(b, button, action) if action == 0 then self:load() end end))
		table.insert(self.window.children, make_button(0.0, 0, 0, "spr_button", 128, 32, "Pause"))
